use super::{OverrideCycles, SampleSheetHeader, SampleSheetVersion};
    ...
    D: Deserializer<'de>,
{
    match u8::deserialize(deserializer)? {
        ...
        other => Err(serde::de::Error::invalid_value(
            serde::de::Unexpected::Unsigned(other as u64),
            &"Unsupported samplesheet version",
        )),
    }
}
...
struct OptionalOverrideCycles(
    #[serde[deserialize_with = "OverrideCycles::deserialize"]] OverrideCycles,
);

pub fn callback_opt<'de, D>(deserializer: D) -> Result<Option<OverrideCycles>, D::Error>
where
    D: Deserializer<'de>,
{
